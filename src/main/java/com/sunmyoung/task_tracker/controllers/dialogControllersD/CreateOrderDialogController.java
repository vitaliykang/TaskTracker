...
public class CreateOrderDialogController {
    private ObservableList<Model> subtasks = FXCollections.observableArrayList();
    ...
    @FXML
    private TableColumn<Model, String> printCol,
            orderCol,
            thicknessCol,
            countCol;
...
//        subtasks.add(subtask);
    ...
    void removePrint(ActionEvent event) {
        Model subtask = tableView.getSelectionModel().getSelectedItem();
...
//        subtaskList.forEach(subtask -> subtask.setTask(task));
    ...
    private void activatePrintCol() {
        ...
        printCol.setOnEditCommit(event -> {
            Model selectedSubtask = event.getTableView().getItems().get(event.getTablePosition().getRow());
    ...
    private Integer getFrameCount() {
        int total = 0;
        for (Model subtask : tableView.getItems()) {
            try {
